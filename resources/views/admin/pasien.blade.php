                ...
                <tbody>
                    @foreach($data as $item)
                        <tr>
                            ...
                            <td>{{ $item->no_rm }}</td>
                            <td>
                                <button class="btn btn-warning btn-sm" onclick="editPasien({{ $item }})">Edit</button>
                                <form action="{{ route('admin.pasien.destroy', $item->id) }}" method="POST" style="display:inline;" id="deleteForm_{{ $item->id }}">
                                    @csrf
                                    @method('DELETE')
